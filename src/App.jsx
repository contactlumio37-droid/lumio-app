...
function RichEditor({value,onChange,placeholder,minHeight=120,accent,th}){
  ...
  return(
    <div style={{border:`1px solid ${th.border2}`,borderRadius:14,overflow:"visible",background:th.inputBg,position:"relative"}}>
      ...
      {/* Bullet spacing fix */}
      <style>{`
        [contenteditable]:empty:before{content:attr(data-placeholder);color:${th.text3};pointer-events:none;}
        [contenteditable] ul{padding-left:16px;margin:4px 0;list-style-type:disc;}
        [contenteditable] ol{padding-left:16px;margin:4px 0;list-style-type:decimal;}
        [contenteditable] li{margin:1px 0;padding-left:2px;display:list-item;}
      `}</style>
    </div>
  );
}
    ...
    <div style={{minHeight:"100vh",background:th.bg,color:th.text,fontFamily:"'Nunito',-apple-system,sans-serif",display:"flex",justifyContent:"center",padding:"18px 16px 90px"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&display=swap');
        *{box-sizing:border-box;}input,textarea{outline:none;}
        input::placeholder,textarea::placeholder{color:${th.label};}
